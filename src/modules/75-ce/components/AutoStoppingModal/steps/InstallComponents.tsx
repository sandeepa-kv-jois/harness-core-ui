...
import DialogExtention, { DialogExtensionContext } from '@connectors/common/ConnectorExtention/DialogExtention'
...
const InstallComponents: React.FC<StepProps<ConnectorInfoDTO> & InstallComponentsProps> = ({
  ...
  const { data: permissionsYaml, loading: yamlLoading } = useMutateAsGet(useCloudCostK8sClusterSetup, {
    ...
    body: {
      connectorIdentifier: (connector || prevStepData)?.spec?.connectorRef,
      featuresEnabled: (connector || prevStepData)?.spec?.featuresEnabled,
      ccmConnectorIdentifier: (connector || prevStepData)?.identifier
    }
  })
